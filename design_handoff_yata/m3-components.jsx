...
// ─── Checkbox (round, M3 Expressive, springy pop) ───
function Check({ checked, color = M3.primary, size = 24 }) {
  return (
    <div style={{
      ...
      flexShrink: 0,
      transition: `background ${DUR.micro}ms ${EASE.standard}, border-color ${DUR.micro}ms, transform ${DUR.micro}ms ${EASE.bounce}`,
      transform: checked ? 'scale(1)' : 'scale(1)',
    }}>
      {checked && (
        <span style={{ display:'flex', animation:`yataPop ${DUR.micro}ms ${EASE.bounce}` }}>
          <Icon name="check" size={size - 8} color={M3.onAccent} />
        </span>
      )}
    </div>
  );
